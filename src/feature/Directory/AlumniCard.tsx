...
export const AlumniCard: React.FC<AlumniCardProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <Typography fontSize={large ? "16px" : medium ? "14px" : "12px"} fontWeight={600}>
          {alumni.name}
        </Typography>
        ...
        <Typography fontSize={large ? "14px" : medium ? "12px" : "10px"}>
          Lokasi Tugas: {alumni.location === "non-makassar" ? "Luar Makassar" : "Makassar"}
        </Typography>
      </Box>
      ...
      {isHoverEnabled && (
        <motion.div
          ...
          animate={hover === index ? { y: 0, opacity: 1 } : { y: "100%", opacity: 0 }}
